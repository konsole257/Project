...
//     return gulp.src(src + '/_view.html')
//       .pipe(access({
...
//       // .pipe(gulp.dest(dist + '/'));
// });
gulp.task('htmlBuild', ['htmlClean', 'htmlSubSub', 'htmlSub', 'htmlMain'], function() {
    return gulp
        .src(dist + '/_view.html')
        .pipe(access({
            accessibilityLevel: 'WCAG2A',
	...
	return gulp
        ...
        .pipe(beautify.html({ indent_size: 4 })) // beautify
		.pipe(gulp.dest(dist + '/')); // HTML 저장
}).task('htmlSubSub', function () {
	return gulp
		.src(src.html.subsub) // HTML 경로
		.pipe(fileInclude({ // Include
				prefix: '@@',
                basepath: './public/src/'
			}))
		.pipe(replace('@@common', subsubRoot)) // 경로 변경
		.pipe(replace('@@sub', subsubRoot)) // 경로 변경
        .pipe(beautify.html({ indent_size: 4 })) // beautify
		.pipe(gulp.dest(dist + '/')); // HTML 저장
...
gulp.task('cssBuild', ['cssClean', 'cssSub', 'cssLib'], function () {
	...
	return gulp
		.src(src.css.sub)
		.pipe(replace('@@css', cssRoot))
		.pipe(concat('airvil.css')) // sub페이지 css를 airvil.css로 Merge합니다.
