	...
	<main id="wrap" class="event">
		...
		<div class="event list" ng-app="event" ng-controller="list">
          ...
          <div class="bbs gallery type2">
						<!-- 이벤트가 없을떄 // -->
						<article class="no_data">예정된 이벤트가 없습니다.<br />다양한 이벤트로 여러분을 찾아 뵙겠습니다!</article>
						<!-- // 이벤트가 없을떄 -->

            <!-- 반복영역 // -->
						<article ng-repeat="event in jsonData | filter : { 'status' : 'ing' } | limitTo : itemMaxLength : itemInit">
              <a href="/event/event_ing_detail.jsp?page={{pageNow}}&amp;detail={{event.index}}">
                <header>
            ...
            <!-- // 반복영역 -->
					...
					<div class="paging" >
    				<a href="/event/event_ing_list.jsp?page={{pagePrev}}" class="prev">이전</a>
    				<span class="num">
    					<a ng-href="/event/event_ing_list.jsp?page={{page.pageNum}}" ng-repeat="page in pageTotal | limitTo : pageMaxLength : pageInit" class="{{page.status}}"><strong>{{page.pageNum}}</strong></a>
    				</span>
    				<a href="/event/event_ing_list.jsp?page={{pageNext}}" class="next">다음</a>
    			</div>
          <!-- -->
				</div>
