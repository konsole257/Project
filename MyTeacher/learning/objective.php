...
<head>
	...
	$(window).load(function(){
		...
		$(document).on('mouseenter', '.hint_area dt:not(.temp):not(.essential)', function(e){ // 2017.08.11 : 수정 : :not(.essential) 추가
			$(this).next('dd').trigger('mouseenter');
		});
		$(document).on('mouseleave', '.hint_area dt:not(.temp):not(.essential)', function(e){ // 2017.08.11 : 수정 : :not(.essential) 추가
			$(this).next('dd').trigger('mouseleave');
		});
		$(document).on('mouseenter', '.hint_area dd:not(.temp):not(.essential)', function(e){ // 2017.08.11 : 수정 : :not(.essential) 추가
		...
		$(document).on('mouseleave', '.hint_area dd:not(.temp):not(.essential)', function(){ // 2017.08.11 : 수정 : :not(.essential) 추가
	...
	<main id="wrap" class="learning objective">
		<div id="contents">
			...
			<section>
				...
				<!-- 2017.08.08 : 추가 // -->
				<!-- fn.unitResult('등급');로 호출 -->
				<div class="result"><!-- fn.unitResult('등급');로 호출, 등급 : tryagain, notbad, excellent, master -->
				...
				<div class="whiteboard">
					...
					<form>
						<fieldset>
							...
							<div class="btn_area">
								<button type="button" class="btn box check" onclick="fn.unitResult('tryagain');">정답확인</button><!-- 2017.07.27 : class변경 -->
					...
					<!-- 힌트 // -->
					<dl class="hint_area">
						<dt class="essential"><span>is you <span>(필요함)</span></span></dt>
                        <dd class="essential">am pushed의 합쳐진 뜻은 밀려집니다가 맞지만 am은 문장의 주인이 I (나는) 일 때 사용</dd>
